...
def main(qtd_features_maximas):
    ...
    pm = 0.025
    selection = 'roulette'
    model1 = BaseGA(problem_dict, epoch, pop_size, pc, pm, crossover='one_point', selection=selection)
    best_position, best_fitness = model1.solve()
    print(f"Solution {qtd_features_maximas}: {best_position}, Fitness: {best_fitness}")

    ## Salvando as features encontradas em um arquivo txt
    with open(f"../Features/AG_features_{qtd_features_maximas}_{best_fitness:.2f}.txt", "w") as f:
    ...
    submission.to_csv(f"../Submissions/Submission{qtd_features_maximas}.csv", index=False)
