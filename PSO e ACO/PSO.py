...
def main(qtd_features_maximas):
    ...
    model1 = BasePSO(problem_dict, epoch, pop_size,  c1, c2, w_min, w_max)
    best_position, best_fitness = model1.solve()
    print(f"SolutionPSO {qtd_features_maximas}: {best_position}, Fitness: {best_fitness}")

    ## Salvando as features encontradas em um arquivo txt
    with open(f"../Features/PSO_features_{qtd_features_maximas}_{best_fitness:.2f}.txt", "w") as f:
    ...
    submission.to_csv(f"../Submissions/SubmissionPSO{qtd_features_maximas}.csv", index=False)
